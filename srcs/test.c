...
int	test_striteri(int detail)
{
	...
	if (strcmp(a, " h t 3 2 5") == 0)
		test_output("ft_striteri", 0, 1);
	else
		test_output("ft_striteri", 1, 1);
	if (detail == 1)
		printf("\n");
	return (0);
}
...
int	test_putendl_fd(int detail)
{
	...
	else if (detail == 1)
	{
		...
		printf("Retour du read() apres avoir ft_putendl_fd la string '%s' dans le fichier test:\n'%s'\n", a, b);
	} 

	/* Test output */
	if (strcmp("Voiture\n", b) == 0)
		test_output("ft_putendl_fd", 0, 1);
	else
		test_output("ft_putendl_fd", 1, 1);
	if (detail == 1)
		printf("\n");
	close(fd);	
...
int	test_putnbr_fd(int detail)
{
	...
	else if (detail == 1)
	{
		...
		printf("Retour du read() apres avoir ft_putnbr '%d' dans le fichier test:\n'%s'\n", a, b);
	} 
	/* Test output */
	if (strcmp("-2147483268", b) == 0)
		test_output("ft_putnbr_fd", 0, 1);
	else
		test_output("ft_putnbr_fd", 1, 1);
	if (detail == 1)
		printf("\n");
	close(fd);	
	/* Reset le fichier */
